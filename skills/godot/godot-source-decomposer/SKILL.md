...
3. Produce a visual-fidelity manifest before proposing code: Theme, fonts, typography, StyleBox state values, texture/icon/shader resources, colors, borders, shadows, outlines, audio feedback, animation parameters, coordinates, offsets, anchors, transforms, size flags, Containers, minimum sizes, viewport/stretch settings, and aspect assumptions. Preserve numerical source values as the component defaults. Exposing those values as optional parameters is compatible with faithful reconstruction, not a redesign choice. Test or reason about landscape, portrait, and at least one alternate resolution; flag absolute coordinates that encode a fragile assumption.
