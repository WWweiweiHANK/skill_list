...
将用户提供的一张卡面、插画或照片制作成可交互的全息/光栅卡：保留主体与原始排版，分离并修复场景、制作分层视差、结构轮廓光，以及可翻转的浏览器预览。用户提到“光栅卡”“光栏卡”“全息卡”“holo card”，或显式调用 `$holo-card` 时使用本 Skill。

输入必须是用户已提供、可访问的单张图像文件。先检查实际图像；不得用示例图代替。典型调用：`$holo-card 把这张图做成光栅卡`。

输出应为一个可审阅的作业目录，其中包含角色、背景、UI、结构四个 RGBA 图层，自包含的交互式 HTML 预览、ZIP 包和来源记录。只有完成透明度、叠层、正反面及正负景深的视觉验收后，才可标记为完成。

## Execution paths

**Codex default:** preserve visible photographic subjects from source pixels, use the built-in image tool only where reconstruction is required, then inspect/repair layers and assemble with local `native.py`. No API key, server or paid API request is required. Never run API setup as a prerequisite.
...
3. Prepare separate **colored** layers:
   - **Photographic character:** author an aligned subject alpha mask, but keep the visible character RGB from `source.png`. Do not ask image generation to recreate, extract or restyle an unobscured photographic subject. Import it with `apply-source-alpha` below.
   - **Character requiring repair:** use generation only for pixels actually concealed by typography or damage, then composite those repaired pixels into the source-preserved character. Unaffected visible subject pixels remain byte-identical to the normalized source.
...
Use the source card for the background and UI references and the final source-preserved character for contours. Preserve canvas, position and scale; do not independently recenter or fit layers. Inpainting is inferred repair, not recovery of unknown original pixels. The first three layers stay colored; only structure is a luminance mask.

4. For a photographic character whose visible pixels do not require repair, import its aligned mask while taking RGB directly from the normalized source:

```sh
python3 <skill-dir>/scripts/native.py apply-source-alpha --job /absolute/job --mask /absolute/character-alpha.png --tool-reference 'Exact mask production and review record'
```

For generated repairs and UI, treat colored generation and alpha preparation as separate stages. Expect RGB/checkerboard output; do not require the image generator to emit an alpha channel, and do not repeatedly request "transparent PNG" to resolve this. If an input already has valid alpha, preserve it. Inspect the actual returned image, identify its checkerboard matte, and prepare the aligned local mask described below. Then import each finished layer:
...
5. Assemble, inspect, approve, and deliver:

```sh
python3 <skill-dir>/scripts/native.py assemble --job /absolute/job
python3 <skill-dir>/scripts/native.py status --job /absolute/job
python3 <skill-dir>/scripts/native.py approve --job /absolute/job --notes 'Actual black/white, tilt, depth, glow and back review'
```

Assembly produces four RGBA assets, self-contained HTML, ZIP and provenance with status `assembled_unreviewed`. Inspect actual results before running `approve`; only explicit approval changes the job to `completed`. No automatic publishing is performed.
...
### Hard gate: prove transparency from file data

An inline preview or visible gray/white checkerboard is never transparency evidence. Immediately pass every character/UI candidate through `native.py add` and read its returned `transparency_evidence` before describing it as a cutout:

- `real-alpha`: the file has an Alpha channel with transparent pixels; continue layer review.
- `opaque-no-alpha` or `opaque-alpha`: call it an opaque candidate, never a transparent PNG. The required state is `needs_alpha_mask`; assembly remains blocked.

Only `apply-source-alpha` or `apply-alpha` may turn an opaque candidate into an imported transparent layer. Their saved RGBA output, mask hash and black/white composites are the review artifacts. Assembly rechecks every imported raw-file hash and must fail if any layer changed after validation; re-import and review the changed layer instead of updating the recorded hash manually.
...
- For a source-preserved photographic character, compare retained RGB against `source.png`; every unaffected visible subject pixel must match. A plausible lookalike, altered silhouette, changed pose or regenerated texture is a rejection even when its alpha looks clean.
